...
transformed data {
  ...
  theta_sim[1] = 1 + abs(normal_rng(0, 0.5));
  theta_sim[3] = 1 + abs(normal_rng(0, 0.5));
  theta_sim[2] = 0.05 + abs(normal_rng(0, 0.05)); 
  theta_sim[4] =0.05 + abs(normal_rng(0, 0.05));  

   real<lower = 0> sigma_sim[2] ;
   sigma_sim[1] = lognormal_rng(-1, 1);
   sigma_sim[2] = lognormal_rng(-1, 1);
   
   real<lower = 0> z_init_sim[2];
  ...
  for (k in 1:2) {
    ...
    for (n in 1:N){
          y_sim[n, k] = lognormal_rng(log(z_sim[n, k]), sigma_sim[k]);
    }
  }
}
